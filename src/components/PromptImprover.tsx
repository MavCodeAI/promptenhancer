...
const PromptImprover = () => {
  ...
  return <div className="w-full max-w-3xl mx-auto p-6 space-y-8">
      <div className="text-center space-y-2">
        <h1 className="font-bold bg-gradient-to-r from-brand-600 to-brand-800 bg-clip-text text-5xl text-[#8f0414]/0">AI Prompt Enhancer</h1>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Textarea placeholder="Enter your prompt here..." value={prompt} onChange={e => setPrompt(e.target.value)} className="min-h-[100px] resize-none bg-gray-50" />
        </div>

        <Button onClick={improvePrompt} className="w-full bg-brand-600 hover:bg-brand-700" disabled={isLoading}>
          {isLoading ? <>
        ...
        {improvedPrompt && <div className="space-y-2 animate-in fade-in-50">
            ...
            <div className="relative">
              ...
              <Textarea value={improvedPrompt} readOnly className="min-h-[100px] resize-none bg-transparent relative z-10" />
            </div>
          </div>}
      </div>
    </div>;
